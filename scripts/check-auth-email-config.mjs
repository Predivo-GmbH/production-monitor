...
async function main() {
  ...
  if (violations.length && !process.env.ALERT_SMTP_HOST) {
    console.error('NOT EMAILED: ' + violations.length + ' project(s) with at-risk auth-email config were found and ALERT_SMTP_HOST is unset, so nobody was told. Wire the mail secrets in this workflow.')
  }
  if (violations.length && process.env.ALERT_SMTP_HOST) {
    try {
